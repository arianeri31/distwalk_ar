...
static struct argp_option argp_client_options[] = {
    ...
    { "output",             OUTPUT_FILE,             "file[,sep=char][,header=bool]",                           0, "Output results to file; format is inferred from file extension (.csv or text otherwise)" },
...
static error_t argp_client_parse_opt(int key, char *arg, struct argp_state *state) {
    ...
    case OUTPUT_FILE: {
        ...
        token = strsep(&output_arg, ",");
        check(token != NULL && token[0] != '\0', "Missing file name in --output argument");
        ...
        while ((token = strsep(&output_arg, ",")) != NULL) {
            if (strncmp(token, "sep=", 4) == 0) {
                ...
                check(0, "Unknown option in --output argument: %s", token);
            }
        }
        break;
    }
...
// is called for every output and will call the correct output format (toher output formats can be added later)
